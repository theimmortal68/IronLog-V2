...
This proves the full loop: clean top-of-range RPE-8 Bench session (155,
2026-08-10 STAB-block seeded baseline) -> run_analysis earns
pending_load_delta=5.0 (increment_ladder[0]) with the tier UNCHANGED (still 0)
-> generate_session prescribes Bench 160 -> commit_session writes
current_load=160 and clears pending_load_delta -> a second clean session ->
165. Plus: no double-apply, miss resets, tier-not-bumped, engine purity.
...
# ---------------------------------------------------------------------------
# THE headline: 155 -> 160 -> 165
# ---------------------------------------------------------------------------

def test_clean_session_ratchets_load_155_160_165(gen_db):
    ...
    # --- Session 1: clean top-of-range RPE-8 @155 ---
    _log_clean_bench(gen_db, session_id=9101, load=155.0)
    ...
    assert st1.current_load == 155, "run_analysis must NOT write current_load (two-writer boundary)"
    ...
    assert prescribed1 == 160, "generation must prescribe the earned 155+5=160"

    st2 = _bench_state(gen_db)
    assert st2.current_load == 160, "commit_session is the sole current_load writer -> 160"
    assert st2.pending_load_delta is None, "commit must clear the marker (apply-exactly-once)"
    assert st2.current_increment_tier == 0

    # --- Session 2: clean again @160 -> 165 ---
    # id well clear of the auto-assigned generated-session ids from the commit above.
    _log_clean_bench(gen_db, session_id=9151, load=160.0, session_date=date(2026, 7, 13))
    ...
    assert prescribed2 == 165, "second clean session must ratchet 160+5=165"

    st4 = _bench_state(gen_db)
    assert st4.current_load == 165
...
def test_no_double_apply_regenerate_without_new_session(gen_db):
    seed_movement_baselines(gen_db)
    _log_clean_bench(gen_db, session_id=9111, load=155.0)
    run_analysis(9111, gen_db, WEEK_KEYER)

    # First generate+commit consumes the delta -> 160, marker cleared.
    assert _generate_and_commit(gen_db) == 160
    assert _bench_state(gen_db).pending_load_delta is None

    # Regenerate WITHOUT logging a new clean session: must stay 160, not 165.
    assert _generate_and_commit(gen_db) == 160
    assert _bench_state(gen_db).current_load == 160
...
def test_rep_miss_earns_no_delta(gen_db):
    seed_movement_baselines(gen_db)
    _log_clean_bench(gen_db, session_id=9121, load=155.0, reps=6, target_reps_high=8)
    ...
    assert st.current_load == 155
    assert _generate_and_commit(gen_db) == 155, "missed session -> load holds at 155"


def test_too_hard_earns_no_delta(gen_db):
    seed_movement_baselines(gen_db)
    # Hit target reps but TOO_HARD -> max_rpe > 8 -> not clean.
    _log_clean_bench(gen_db, session_id=9131, load=155.0, feedback=FeedbackTap.TOO_HARD)
