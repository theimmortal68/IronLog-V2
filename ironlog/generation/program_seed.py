...
def _seed_d1(db: Session, pd: ProgramDay, lib: Dict[str, int]) -> None:
    ...
    t3 = _add_tier(db, pd.id, "T3 GS", 3, TierKind.GIANT_SET, rounds=3)
    ...
    _add_te(db, t3.id, "d1_t3b", "Cross-Body Lateral Raise", lib, 2, "free",
            pattern="lateral_raise", rep_low=10, rep_high=12,
            scheme="DOUBLE_PROGRESSION")
    ...
    t4 = _add_tier(db, pd.id, "T4 GS", 4, TierKind.GIANT_SET, rounds=3)
...
def _seed_d2(db: Session, pd: ProgramDay, lib: Dict[str, int]) -> None:
    ...
    t2 = _add_tier(db, pd.id, "T2 GS", 3, TierKind.GIANT_SET, rounds=3)
    ...
    t3 = _add_tier(db, pd.id, "T3", 4, TierKind.PAIR, rounds=1)
...
def _seed_d5(db: Session, pd: ProgramDay, lib: Dict[str, int]) -> None:
    ...
    t3 = _add_tier(db, pd.id, "T3 GS", 4, TierKind.GIANT_SET, rounds=3)
...
def _seed_d6(db: Session, pd: ProgramDay, lib: Dict[str, int]) -> None:
    ...
    gs1 = _add_tier(db, pd.id, "GS1", 1, TierKind.GIANT_SET, rounds=3)
    ...
    gs2 = _add_tier(db, pd.id, "GS2", 2, TierKind.GIANT_SET, rounds=3)
    ...
    gs3 = _add_tier(db, pd.id, "GS3", 3, TierKind.GIANT_SET, rounds=3)
